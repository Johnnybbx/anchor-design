...
st.set_page_config(page_title="Anchor Layout (X Label Outside)", layout="centered")
st.title("🔩 錨栓配置圖（X 向標註樣式與 Y 向一致）")

st.markdown("此版本將 X 向單段距離標註移至箭頭外側，與 Y 向一致，避免擁擠。")
...
if n_x > 1:
    ...
    for j in range(n_x - 1):
        ...
        ax.text(x1 + 10, y_spacing_line, f"{spacing_x:.0f} mm", va='center', fontsize=label_fontsize)
...
st.caption("※ X 向單段距離標註已改為放置箭頭右側，與 Y 向一致，視覺清爽。")
